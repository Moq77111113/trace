...
import { createRunApi, type RunApi, type UploadedAttachment, type Verdict } from './api';

type RunData    = NonNullable<RunPageData>;
...
export class LiveRunController {
  ...
  constructor(data: RunData, api: RunApi = createRunApi(data.run.id)) {
    this.runId      = data.run.id;
    this.scenarios  = data.scenarios.slice();
    this.notes      = data.run.notes ?? '';
