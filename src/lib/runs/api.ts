...
export function createRunApi(runId: string) {
  return {
    async patchScenarioStatus(scenarioId: string, status: Verdict): Promise<void> {
      const res = await fetch(`/api/runs/${runId}/scenarios/${scenarioId}`, {
        method:  'PATCH',
    ...
    async uploadScenarioAttachment(scenarioId: string, file: File): Promise<UploadedAttachment> {
      ...
      const res = await fetch(`/api/runs/${runId}/scenarios/${scenarioId}/attachments`, {
        method: 'POST',
        body:   form,
      });
