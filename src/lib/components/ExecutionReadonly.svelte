<script lang="ts">
  ...
  const onRerunSubmit: SubmitFunction = () => {
    rerunning  = true;
    rerunError = null;
    return async ({ result, update }) => {
      ...
      if (result.type === 'failure') {
        rerunError = (result.data as { error?: string } | undefined)?.error ?? `Re-run failed (${result.status})`;
      } else if (result.type === 'error') {
        rerunError = result.error.message;
      }
    };
  };
  ...
  const selectedSteps       = $derived(selected ? extractScenarioSteps(data.execution.featureContentAtStart, selected.scenarioName) : []);
  const runKind             = $derived(toStatusKind(data.execution.status));
</script>
  ...
  <header class="flex flex-col gap-2.5 px-5 py-3.5 border-b border-border bg-bg max-md:px-3.5">
    <div class="flex items-center gap-3.5 flex-wrap">
      <div class="flex items-center gap-2.5">
        ...
        <Pill kind={runKind}>{data.execution.status.toLowerCase()}</Pill>
      </div>

      <div class="flex items-center gap-3 text-[12px] text-ink-3 tabular-nums flex-wrap">
        <span class="font-mono text-ink-2">#{data.execution.id.slice(0, 8)}</span>
        ...
        {/if}
        <span class="w-[3px] h-[3px] rounded-full bg-ink-mute"></span>
        <span>{new Date(data.execution.startedAt).toLocaleString()}</span>
        <span class="w-[3px] h-[3px] rounded-full bg-ink-mute"></span>
        <span>{formatExecutionDuration(data.execution.startedAt, data.execution.finishedAt)}</span>
        <span class="w-[3px] h-[3px] rounded-full bg-ink-mute"></span>
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        {#if failedCount > 0}
          <form method="POST" action="?/rerunFailed" use:enhance={onRerunSubmit} class="inline-flex">
        ...
        {/if}
        <ExecutionLauncher projectId={data.project.id} featureId={data.feature.id} />
      </div>
      {#if rerunError}
        <span class="text-[12px] text-fail-ink" role="alert">{rerunError}</span>
      {/if}
    </div>
    ...
    <article class="overflow-auto min-h-0 px-5 py-4 max-md:px-3.5">
      {#if selected}
        ...
        {#if selectedSteps.length > 0}
          <div class="mb-4">
            <ScenarioSteps steps={selectedSteps} scenarioStatus={selected.status} />
          </div>
        {/if}
