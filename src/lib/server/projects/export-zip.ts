...
import { prependTraceTag } from '$lib/features/feature-import/lib/trace-tag';
...
export async function buildExportEntries(projectId: string): Promise<ExportEntry[] | null> {
  ...
  for (const f of rows) {
    ...
    const featureCode = formatFeatureCode(project.codePrefix, f.codeSeq);
    const tagged      = prependTraceTag(f.content, featureCode);
    const groupName   = f.groupId ? groupNamesById.get(f.groupId) : null;
    const content     = groupName ? prependGroupMeta(tagged, groupName) : tagged;
...
export async function streamProjectZip(projectId: string): Promise<ProjectArchive | null> {
  const entries = await buildExportEntries(projectId);
