<script lang="ts">
	...
	import ForgotPasswordForm from '$lib/features/password-recovery/ui/ForgotPasswordForm.svelte';
...
<PageTitle title={m.page_title_forgot_password()} />
...
<section class="min-h-screen flex items-center justify-center bg-canvas px-4">
	<div class="w-full max-w-sm bg-surface border border-border rounded-xl p-6 shadow-[var(--shadow-2)]">
		...
		<h1 class="text-[18px] font-semibold tracking-tight mb-1">{m.page_title_forgot_password()}</h1>

		<ForgotPasswordForm done={Boolean(form?.done)} email={form?.email ?? ''} />

		<a href="/login" class="block mt-5 text-[12px] text-ink-3 hover:text-ink underline-offset-2 hover:underline">
