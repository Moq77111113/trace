...
describe('createGroup + listGroups', () => {
  ...
  it('returns duplicate-name error when name is taken', async () => {
    ...
    expect(dup).toEqual({ ok: false, error: 'duplicate-name' });
  });
...
describe('renameGroup', () => {
  it('renames a group', async () => {
    ...
    const g = unwrap(await createGroup({ projectId: p.id, name: 'Old' }));
...
describe('deleteGroup', () => {
  it('deletes a group and reports the number of features moved to ungrouped', async () => {
    ...
    const g = unwrap(await createGroup({ projectId: p.id, name: 'Auth' }));
    ...
    expect(result).toEqual({ ok: true, value: { affectedFeatureCount: 2 } });
...
describe('feature create/save with groupId', () => {
  it('createFeature accepts a same-project groupId', async () => {
    ...
    const g = unwrap(await createGroup({ projectId: p.id, name: 'X' }));
  ...
  it('createFeature rejects a foreign-project groupId', async () => {
    ...
    const g  = unwrap(await createGroup({ projectId: p2.id, name: 'X' }));
