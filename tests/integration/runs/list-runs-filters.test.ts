...
import { listRunEnvironments, listRunsForProject } from '$lib/server/runs/queries';
...
async function seedRun(
  featureId: string,
  content: string,
  attrs: { status: 'PASSED' | 'FAILED' | 'SKIPPED' | 'RUNNING'; source: 'MANUAL' | 'CI'; environment: string | null; startedAt?: Date; executedBy?: string },
) {
  const [r] = await db
    .insert(runs)
    .values({
      ...
      featureContentAtRun: content,
      status:              attrs.status,
      startedAt:           attrs.startedAt ?? new Date(),
      finishedAt:          attrs.status === 'RUNNING' ? null : (attrs.startedAt ?? new Date()),
    })
    .returning();
...
describe('listRunsForProject — filters', () => {
  it('filters by status, source, environment', async () => {
    ...
    const byStatus = await listRunsForProject(p.id, { status: 'FAILED' });
    expect(byStatus.total).toBe(1);
    expect(byStatus.rows[0]?.source).toBe('CI');

    const bySource = await listRunsForProject(p.id, { source: 'CI' });
    expect(bySource.total).toBe(2);

    const byEnv = await listRunsForProject(p.id, { environment: 'staging' });
    expect(byEnv.total).toBe(2);

    const combined = await listRunsForProject(p.id, { source: 'CI', environment: 'staging' });
  ...
  it('filters by feature and group', async () => {
    ...
    const byFeature = await listRunsForProject(p.id, { featureId: fA.id });
    expect(byFeature.total).toBe(1);
    expect(byFeature.rows[0]?.featureName).toBe('In A');

    const byGroup = await listRunsForProject(p.id, { groupId: gB.id });
    expect(byGroup.total).toBe(1);
    expect(byGroup.rows[0]?.featureName).toBe('In B');

    const ungrouped = await listRunsForProject(p.id, { groupId: 'ungrouped' });
  ...
  it('orders by startedAt desc and paginates', async () => {
    ...
    const firstPage = await listRunsForProject(p.id, { pageSize: 2, page: 1 });
    expect(firstPage.total).toBe(5);
    expect(firstPage.rows.map((r) => r.executedBy)).toEqual(['Run 4', 'Run 3']);

    const secondPage = await listRunsForProject(p.id, { pageSize: 2, page: 2 });
    expect(secondPage.rows.map((r) => r.executedBy)).toEqual(['Run 2', 'Run 1']);

    const thirdPage = await listRunsForProject(p.id, { pageSize: 2, page: 3 });
    expect(thirdPage.rows.map((r) => r.executedBy)).toEqual(['Run 0']);
  });

  it('listRunEnvironments returns distinct non-null envs', async () => {
    const p = await seedProject();
    const f = await seedFeature(p.id, 'Login');
    ...
    const envs = await listRunEnvironments(p.id);
